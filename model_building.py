...
from config import SAMPLE_SIZE, N_FEATURES, DATA_LIST, RESULTS_FILE, KERNEL_USE, BOOTSTRAP, N_BOOTSTRAP, N_SAMPELS
...
# For tracking pure clusters across methods
# TODO: reset after each dataset.
...
def preprocess_data(
    ...
    Preprocess the given dataframe:
      ...
      5. Encode categorical features if present.
      6. Identify the problem type (binary, multiclass, regression).

    Args:
        df (pd.DataFrame): Original dataset.
        target_col (str): Name of the target column.
        dataset_name (str): Dataset name used for folder naming.
        sample_size (int): Maximum number of samples to retain.
        n_features (int): Maximum number of features to use.

    Returns:
        tuple: (X_train, X_test, y_train, y_test, model_type, num_classes)
    """
...
def apply_clustering_method(data: np.ndarray, method: str = "kmedoids", n_clusters: int = 3, random_state: int = 42):
    ...
    Available methods:
      - "kmedoids": Uses KMedoids from sklearn_extra.
      - "kmeans": Uses KMeans clustering.
      - "hierarchical": Uses Agglomerative Clustering.
      - "gmm": Uses Gaussian Mixture Models.
      - "quantile": Uses quantile-based binning (only for 1D data).

    Args:
        data (np.ndarray): Data to cluster, shape (n_samples, n_features).
    ...
    if method.lower() == "kmedoids":
        ...
        # DBSCAN determines the number of clusters based on eps/min_samples.
        # n_clusters and random_state do not apply here.
...
def compute_xper_values_and_cluster(
    ...
    # Compute XPER values. The kernel parameter is passed.
    xper_instance = ModelPerformance(X.values, y.values, X.values, y.values, model, sample_size=X.shape[0])
    phi_global, phi_per_instance = xper_instance.calculate_XPER_values(["AUC"], kernel=KERNEL_USE)
    
    # Save XPER values for training set.
...
def cluster_data_by_error(y_true: pd.Series, y_pred: pd.Series, min_clusters: int = 2, max_clusters: int = 5, method: str = "kmeans", random_state: int = 42) -> np.ndarray:
    ...
    This function supports different clustering methods (default is kmeans).

    Args:
        y_true (pd.Series): True target values.
        y_pred (pd.Series): Predicted values.
        n_clusters (int): Number of clusters desired.
        method (str): Clustering method for errors ("kmeans", "quantile", etc.).
        random_state (int): Random seed.
...
def bootstrap_xper_confidence_intervals(
    ...
    """
    Compute bootstrap confidence intervals for the global XPER values.
    Inspired by the methods in Williamson and Feng (2020) for statistical
    inference of Shapley-based importance measures.

    Args:
        X (pd.DataFrame): Training features.
        y (pd.Series): Training target.
        model: Trained model.
        metric (str): Metric to use ("AUC" or "R2").
        n_bootstraps (int): Number of bootstrap iterations.
        alpha (float): Significance level for the CI (e.g., 0.05 for 95% CI).
        kernel (str, optional): Kernel parameter for XPER if needed.
    ...
    for i in range(n_bootstraps):
        ...
        try:
            ...
            boot_global, _ = boot_instance.calculate_XPER_values([metric], kernel=kernel)
...
def train_and_evaluate_cluster_models(X_train, y_train, target_col, cluster_labels, model_prefix):
    ...
    Args:
        ...
        model_prefix (str): Identifier for the clustering method ("xper", "feature", "epsilon").
    
    Returns:
        tuple: (cluster_metrics, label_encoders)
            - cluster_metrics: dict mapping cluster_id to a dict of computed metrics.
            - label_encoders: dict of LabelEncoder objects for non-pure clusters.
    """
    ...
    for cluster_id in np.unique(cluster_labels):
        ...
        #logger.debug(f"Original classes: {np.sort(y_train_cluster[target_col].unique())}")
        #logger.debug(f"Encoded classes: {np.sort(np.unique(y_train_cluster_encoded))}")
...
def _compute_test_metrics(y_true, y_pred, y_proba, cluster_id):
    ...
    Args:
        ...
        y_proba (np.array): Predicted probability for class 1.
        cluster_id (int): Identifier for the cluster.
    
    Returns:
        dict: Metrics including "AUC Score", "Accuracy", "Precision", "Recall (TPR)",
              "F1 Score", "Log Loss", "Brier Score", "KS Statistic", "Cluster Size",
              "Pure Cluster", and "Cluster ID".
    """
    unique_labels = np.unique(y_true.values)
    is_pure = (len(unique_labels) == 1)
    
    # AUC cannot be computed for a pure cluster.
    if not is_pure and y_proba is not None:
        try:
    ...
    # Log loss and Brier score (if probabilities are provided)
    try:
        logloss = log_loss(y_true.values, y_proba) if y_proba is not None else None
    except Exception:
        logloss = None
    try:
        brier = brier_score_loss(y_true.values, y_proba) if y_proba is not None else None
    except Exception:
        brier = None

    accuracy = accuracy_score(y_true.values, y_pred)
    precision = precision_score(y_true.values, y_pred, zero_division=0) #TODO: dont linke the zero division. instead make none
...
def evaluate_test_data_xper_clusters(X_test, y_test, xper_scaler, label_encoders):
    ...
    xper_instance = ModelPerformance(X_test.values, y_test.values, X_test.values, y_test.values, baseline_model, sample_size=X_test.shape[0])
    phi_global_test, phi_per_instance_test = xper_instance.calculate_XPER_values(["AUC"], kernel=KERNEL_USE)
...
def plot_epsilon_violin(epsilon_train: np.ndarray, epsilon_test: np.ndarray, save_path: str):
    ...
    Args:
        epsilon_train (np.ndarray): 1D array of training error values.
        epsilon_test (np.ndarray): 1D array of test error values.
        save_path (str): Directory where the plot will be saved as "epsilon_distribution_violin.png".
    """
...
def process_single_dataset(dataset_name: str, df: pd.DataFrame, target_col: str):
    ...
    Execute the entire pipeline for a single dataset:
      ...
      3. Compute XPER-based clustering.
      4. Compute feature-based clustering.
      5. Compute epsilon-based clustering.
      6. Train cluster-specific models for each clustering approach.
      7. Evaluate on the test set.
      8. Compute bootstrap confidence intervals for XPER values.
      9. Summarize results.
    
    Returns:
        dict: Summary of results for the dataset.
    """
    ...
    baseline_eval = _compute_test_metrics(y_test, y_pred_test, y_test_proba, cluster_id="Baseline")
    ...
    xper_cluster_labels, xper_best_n_clusters, xper_best_score, xper_scaler = compute_xper_values_and_cluster(
        X_train, y_train, baseline_model, min_clusters=2, max_clusters=2, method="kmedoids"
    )
    ...
    feature_cluster_labels, feature_best_n_clusters, feature_best_score, feature_scaler = cluster_data_by_features(
        X_train, min_clusters=2, max_clusters=2, method="kmedoids"
    )
    ...
    epsilon_cluster_labels, epsilon_best_n_clusters, epsilon_best_score, epsilon_scaler = cluster_data_by_error(
            y_train, y_pred=y_train_proba, min_clusters=2, max_clusters=2, method="kmeans"
...
def run_experiments() -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: A DataFrame summarizing the results for all datasets.
    """
...
if __name__ == '__main__':
    ...
    #for SAMPLE_SIZE in tqdm([1000, 2000]):
    #    for N_FEATURES in tqdm([5, 4]):
