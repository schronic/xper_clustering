...
def XPER_choice(y, X, model, Eval_Metric, var_interet=None, N_coalition_sampled=1000,
                ...
                execution_type="ProcessPoolExecutor"): # Add the kernel parameter
    ...
    if kernel == True:
        ...
        logger.info(len(combination_list_sampled))
    else:
    ...
    with executor_class(max_workers=60) as executor:
        ...
        for result in tqdm(concurrent.futures.as_completed(results), total=len(results), desc="Processing"):
            res = result.result()
            s = res[0]
            weight[s, :] = res[1]
            Metric[s] = res[2]
