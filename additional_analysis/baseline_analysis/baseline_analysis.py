...
def load_experiment_data(exp_path):
    ...
    # Load global XPER values.
    global_train_xper_file = os.path.join(exp_path, 'xper_values', 'train_global_xper.csv')
    if os.path.exists(global_train_xper_file):
        data['global_train_xper'] = pd.read_csv(global_train_xper_file)
    else:
        logging.warning(f"Global XPER file not found: {global_train_xper_file}")
        data['global_train_xper'] = None

    # Load individual XPER values.
    individual_train_xper_file = os.path.join(exp_path, 'xper_values', 'train_per_instance_xper.csv')
    if os.path.exists(individual_train_xper_file):
        individual_train_xper = pd.read_csv(individual_train_xper_file, index_col=0)
        clusters_file = os.path.join(exp_path, 'xper_values', 'train_xper_clusters.csv')
        if os.path.exists(clusters_file):
            xper_clusters = pd.read_csv(clusters_file, index_col=0)
            merged_train_xper = xper_clusters.merge(individual_train_xper, how='inner', left_index=True, right_index=True)
        else:
            merged_train_xper = individual_train_xper
        data['individual_train_xper'] = merged_train_xper
    else:
        logging.warning(f"Individual XPER file not found: {individual_train_xper_file}")
        data['individual_train_xper'] = None

    # Load global XPER values for test set.
    global_test_xper_file = os.path.join(exp_path, 'xper_values', 'test_global_xper.csv')
    if os.path.exists(global_test_xper_file):
        data['global_test_xper'] = pd.read_csv(global_test_xper_file)
    else:
        logging.warning(f"Global XPER file not found: {global_test_xper_file}")
        data['global_test_xper'] = None

    # Load individual XPER values for test set.
    individual_test_xper_file = os.path.join(exp_path, 'xper_values', 'test_per_instance_xper.csv')
    if os.path.exists(individual_test_xper_file):
        data['individual_test_xper'] = pd.read_csv(individual_test_xper_file, index_col=0)
    else:
        logging.warning(f"Individual XPER file not found: {individual_test_xper_file}")
        data['individual_test_xper'] = None

    # Load baseline model.
    baselin_model_path = os.path.join(exp_path, 'models', 'baseline_model.pkl')
    baseline_model = joblib.load(baselin_model_path)
    ...
    # Compute SHAP values.
    explainer = shap.Explainer(baseline_model, seed=42)
    ...
    perm_importance_train = permutation_importance(baseline_model, X_train, y_train,
                                           n_repeats=30, random_state=42, scoring='roc_auc')
    data['perm_importance_train'] = perm_importance_train

    perm_importance_test = permutation_importance(baseline_model, X_test, y_test,
                                           n_repeats=30, random_state=42, scoring='roc_auc')
...
def compare_experiments(exp_data_list, metric_name, output_dir):
    ...
    for exp_name, data in exp_data_list:
        ...
        if df is not None:
            ...
            logging.info(f"No {metric_name} data for experiment {exp_name}")
...
def compare_global_importance_metrics(exp_data_list, output_dir):
    ...
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        #logging.info(f"Created output directory for global comparison: {output_dir}")
...
def compare_xper_feature_distribution(data, output_dir, experiment_name):
    ...
    if individual_train_df is None or individual_test_df is None or X_train is None or X_test is None:
        logging.info(f"Insufficient data for XPER vs feature distribution comparison in experiment {experiment_name}")
        return
    ...
    for feature in features:
        ...
        plt.savefig(cmp_file, dpi=300, bbox_inches='tight')
        plt.close()
        #logging.info(f"Saved normalized feature distribution comparison for {feature} in {experiment_name} to {cmp_file}")
...
def stratified_analysis(individual_xper_df, strat_col, output_dir, experiment_name):
    ...
    if strat_col not in individual_xper_df.columns:
        logging.warning(f"Stratification column '{strat_col}' not found in experiment {experiment_name}.")
        return
...
def main():
    ...
    for exp_path in exp_paths:
        ...
        data = load_experiment_data(os.path.join(BASE_EXPERIMENT_DIR, exp_path, 'credit_risk'))
        exp_data_list.append((exp_name, data))

    # Create a main output directory for all analysis plots.
    output_dir = os.path.join(BASE_EXPERIMENT_DIR, "analysis_plots")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    ...
    for exp_name, data in exp_data_list:
        ...
        if individual_df is not None:
            ...
            if 'Cluster' in individual_df.columns:
                #logging.info(f"Performing stratified analysis for experiment {exp_name}")
                stratified_analysis(individual_df, 'Train Cluster', exp_output_dir, exp_name)
        else:
            logging.info(f"No individual XPER data for experiment {exp_name}")
        ...
        if individual_df_test is not None:
            ...
            logging.info(f"No individual XPER test data for experiment {exp_name}")
        ...
        if global_df is not None:
            ...
            logging.info(f"No global XPER train data for experiment {exp_name}")
        ...
        if global_df_test is not None:
            ...
            logging.info(f"No global XPER test data for experiment {exp_name}")
